...
beautiful.init("/home/brandon/.config/awesome/themes/dust/theme.lua")
...
-- Register widget
vicious.register(weatherwidget, vicious.widgets.weather, "${tempf}°, ${sky}", 3600, "KSWO")
-- }}}
...
wibox_top[s].widgets = {
	...
	separator, cpugraph.widget, tzswidget, cpuicon, s == 1 and mysystray or nil,
	tasklist[s],
...
globalkeys = awful.util.table.join(
    ...
    -- Standard program, keybindings, shortcuts
...
awful.rules.rules = {
	...
	{ rule = { class = "taskpaper" }, properties = { tags[1][5]}},
}
-- }}}
...
-- Commands to be run everytime awesome is restarted
do
  local cmds = 
  { 
    "nitrogen --restore",
...
-- commands to be run only once per awesome session

function run_once(prg)
  awful.util.spawn_with_shell("pgrep -u brandon -x " .. prg .. " || (" .. prg .. ")")
end
...
--run_once("synergyc 192.168.1.232")
